...
struct SpotsView: View {
    ...
    var body: some View {
        NavigationStack {
            Group {
                if !authVM.isGerente,
                   let activeSpot = vm.userActiveSpot(for: authVM.currentUserEmail ?? "") {
                    activeSpotBlockView(spot: activeSpot)
                } else if !authVM.isGerente, vm.totalAvailable == 0, !vm.spots.isEmpty {
                    allOccupiedView
                } else {
                    spotsScrollView
                }
